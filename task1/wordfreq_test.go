...
func TestWordFrequency(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "punctuation only tokens ignored",
			input: "... !! hello ?? world",
		...
		},
		{
			...
			want:  map[string]int{"version": 2, "2": 2},
		},
		{
			name:  "hyphen separates words",
			input: "well-known well-known",
			want:  map[string]int{"well": 2, "known": 2},
		},
		{
			name:  "apostrophes ignored",
			input: "it's it's",
			want:  map[string]int{"its": 2},
		},
		{
			name:  "internal punctuation ignored",
